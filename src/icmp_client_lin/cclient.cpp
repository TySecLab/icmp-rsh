...
CClient::~CClient()
{
	...
	if (m_thd_recv != nullptr)  delete m_thd_recv;
	if (m_thd_input != nullptr) delete m_thd_input;
	...
	else if(p_header->cmd == SERVER_REP_RESULT)
	{
		if (p_header->sid == m_sid)
		{
			...
			if (data_size > 0)
			{
				...
				write(1, dec_buff, data_size);

				////解压缩
				//char* dst = NULL;
				//if (clz4::decompress(dec_buff, data_size - 1, p_header->src_size, &dst) == 0)
				//{
				//	write(1, dst, p_header->src_size);
				//	delete[] dst;
				//}
...
void CClient::tfun_get_input_cb(void* param)
{
	...
	while (p_this->m_rcv_running)
	{
		...
		if (strlen(input) > 0)
		{
			...
			p_this->m_cape->encrypt((unsigned char*)input, strlen(input), (unsigned char*)enc_buff,random());
