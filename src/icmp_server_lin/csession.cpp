...
void CSession::tfun_resolve_recv_cb(void* param)
{
	...
	while (p_this->m_thd_running)
	{
		if (p_this->m_list_recv_pack.empty())
		{
			usleep(sleep_usec);
			loop++;
			if (loop * sleep_usec >= 1000 * 1000 * 60 * 5)
			{
		...
		{
			if (p_this->recv_pack((char*)p_this->m_list_recv_pack.front().c_str(), p_this->m_list_recv_pack.front().size()) < 0)
			{
				p_this->m_thd_running = 0;
				p_this->m_delete_cb(p_this->m_sid);
				break;
...
int CSession::slice_data_to_pack(short cmd, char* data, int size)
{
	...
	//	int compress_size = 0;
	//	if (clz4::compress(data, size, &compress_data, &compress_size) < 0)
	//	{
	//		return 0;
	//	}
	...
	//	//申请单个包大小
	//	std::string tmp;
	//	tmp.resize(sizeof(MY_TRANS_HEADER) + compress_size + 1);

	//	//设置header
	//	MY_TRANS_HEADER* p_hdr = (MY_TRANS_HEADER*)tmp.c_str();
	//	p_hdr->cmd = cmd;
	//	p_hdr->sid = m_sid;
	//	p_hdr->pack_size = sizeof(MY_TRANS_HEADER) + compress_size + 1;
	...
	//	//加密数据
	//	m_cape->encrypt((unsigned char*)compress_data, compress_size, (unsigned char*)tmp.c_str() + sizeof(MY_TRANS_HEADER),random());
	//	delete[] compress_data;

	//	//复制数据
	//	//memcpy((char*)tmp.c_str() + sizeof(MY_TRANS_HEADER), data, size);
	...
	else
	{
		...
		for (int i = 0; i < pack_count; i++)
		{
			...
			if (pack_count == 1)
			{
				...
				//加密数据
				m_cape->encrypt((unsigned char*)data, size, (unsigned char*)tmp.c_str() + sizeof(MY_TRANS_HEADER), random());
			...
			else
			{
				...
				if (i < pack_count - 1)
				{
					...
					//加密数据
					m_cape->encrypt((unsigned char*)data + (i * MAX_PACK_SIZE), MAX_PACK_SIZE, (unsigned char*)tmp.c_str() + sizeof(MY_TRANS_HEADER), random());
				}
				else
				{
					...
					//加密数据
					m_cape->encrypt((unsigned char*)data + (i * MAX_PACK_SIZE), size - (i * MAX_PACK_SIZE), (unsigned char*)tmp.c_str() + sizeof(MY_TRANS_HEADER), random());
				}
			}
